...
- App startup and app-language changes now sync the current hotel language to `/pms/setUserLang?lang=...` so hotel backend response messages follow the active locale. The language codes use the existing hotel mapping: Japanese `JP`, English `EN`, and Chinese `CH`.
