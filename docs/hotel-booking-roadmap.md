...
- Hotel booking confirmation coupon selection uses `/pms/coupons/order/custListV2` grouped by each coupon's `use` flag. Selecting or clearing a coupon re-quotes through `/pms/order/room/extraPerson` with `couponsCounts`, and the selected coupon is passed into `/pms/bookingorder/save/v2`.
