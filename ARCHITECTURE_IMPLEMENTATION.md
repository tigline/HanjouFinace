...
- 以后实现 UI 时，样式选择路径必须固定为：先对照设计要求，检查 `core_ui_kit` 现有主题语义与样式是否已覆盖；若已有合适的颜色和字体样式，必须直接复用；若没有，再先补充到主题/样式定义后再使用；禁止在业务页面或组件内直接硬编码颜色、字号、字重等视觉样式。
- 通用控件（弹框、底部弹框等）统一由 `core_ui_kit` 提供，不在业务侧重复实现。
- 业务页面优先复用 SDK 组件，再做业务差异化皮肤。
- View（page/widget）默认只负责布局、样式与事件绑定，不承载可复用业务逻辑。
- 金融/交易类 App 的页面级读取数据（账户余额、入出金、订单、持仓、收益、通知详情等）默认采用“短缓存”策略：`FutureProvider/StreamProvider` 优先使用 `autoDispose`，页面退出即释放，重新进入时重新拉取，避免沿用上一次失败态或旧数据。
- 若某页面属于根 Tab 常驻页，且确实需要缓存以减少闪烁或请求频率，必须显式说明原因并配套刷新策略（如下拉刷新、显式 `invalidate`、登录后统一刷新、前后台切换刷新），不能默认长时间保留旧态。
- 根 Tab 常驻页与交易页必须补齐三类刷新触发：登录成功后统一 `invalidate`、App 从后台回到前台时刷新、页面重新聚焦（如返回上一级/切回当前 Tab）时刷新，保证金融与交易数据的时效性。
